...
export class ApiKeyHeader extends TranslationMixin(LitElement) {
    ...
    _renderInstallRequiredState() {
        return html` <button class="ollama-action-button install" @click=${this.ensureOllamaReadyWithUI}>${this.t('apiKey.installOllama')}</button> `;
    }

    _renderStartRequiredState() {
        return html` <button class="ollama-action-button start" @click=${this.ensureOllamaReadyWithUI}>${this.t('apiKey.startOllama')}</button> `;
    }
    ...
    _renderReadyState() {
        return html`
            ...
            <input
                type="text"
                class="api-input"
                placeholder="${this.t('apiKey.modelNamePlaceholder')}"
                ...
                ${this.getCombinedModelSuggestions().map(
                    model => html`
                        <option value=${model.name}>
                            ${model.name} ${model.status === 'installed' ? this.t('settings.installed') : model.status === 'history' ? '📝 Recent' : '- Available'}
                        </option>
                    `
                )}
            </datalist>
    ...
    _renderFailedState(state) {
        return html`
            <div style="margin-top: 6px; padding: 8px; background: rgba(239,68,68,0.1); border-radius: 8px;">
                <div style="font-size: 11px; color: rgba(239,68,68,0.8); margin-bottom: 4px; text-align: center;">${this.t('apiKey.connectionFailed')}</div>
                <div style="font-size: 10px; color: rgba(239,68,68,0.6); text-align: center; margin-bottom: 6px;">
                    ${state.message || this.t('apiKey.unknownState')}
                </div>
                <button
                    class="action-button"
    ...
    _renderUnknownState() {
        return html`
            <div style="margin-top: 6px; padding: 8px; background: rgba(255,200,0,0.1); border-radius: 8px;">
                <div style="font-size: 11px; color: rgba(255,200,0,0.8); text-align: center;">${this.t('apiKey.unknownState')}</div>
            </div>
        `;
    }
            ...
            <div class="container">
                ...
                <div class="header">
                    <div class="back-button" @click=${this.handleBack}>
                        ...
                        <div class="back-button-text">${this.t('apiKey.back')}</div>
                    </div>
                    <div class="title">${this.t('apiKey.title')}</div>
                </div>

                <!-- LLM Section -->
                <div class="section">
                    <div class="row">
                        <div class="label">${this.t('apiKey.selectLLMProvider')}</div>
                    ...
                    <div class="row">
                        <div class="label">${this.t('apiKey.enterAPIKey')}</div>
                            ...
                            : html`
                                  <div class="input-wrapper">
                                      <input
                                          ...
                                          placeholder="${this.t('apiKey.enterYourKey', { provider: llmProviderName })}"
                ...
                <div class="section">
                    <div class="row">
                        <div class="label">${this.t('apiKey.selectSTTProvider')}</div>
                        <div class="provider-selector">
                              ...
                              ? html`
                                    <div class="input-wrapper">
                                        <select
                                            ...
                                            ?disabled=${this.isLoading}
                                        >
                                            <option value="">${this.t('apiKey.selectModel')}</option>
                                            ${[
                                                { id: 'whisper-tiny', name: this.t('apiKey.whisperTiny') },
                                                { id: 'whisper-base', name: this.t('apiKey.whisperBase') },
                                                { id: 'whisper-small', name: this.t('apiKey.whisperSmall') },
                                                { id: 'whisper-medium', name: this.t('apiKey.whisperMedium') },
                              ...
                              : html`
                                    <div class="input-wrapper">
                                        <input
                                            type="password"
                                            class="api-input ${this.sttError ? 'invalid' : ''}"
                                            placeholder="${this.t('apiKey.enterSTTKey')}"
                ...
                <div class="confirm-button-container">
                    <button class="confirm-button" @click=${this.handleSubmit} ?disabled=${isButtonDisabled}>
                        ${this.isLoading
                            ? this.t('apiKey.settingUp')
                            : this.installingModel
                              ? this.t('apiKey.installing', { model: this.installingModel })
                              : Object.keys(this.whisperInstallingModels).length > 0
                                ? this.t('apiKey.downloading')
                                : this.t('apiKey.confirm')}
                    </button>
                </div>

                <div class="footer">
                    ${this.t('apiKey.getAPIKey')}
                    <br />
                    Lucide ne collecte pas vos données personnelles —
                    <span class="footer-link" @click=${this.openPrivacyPolicy}>${this.t('apiKey.seeDetails')}</span>
                </div>
